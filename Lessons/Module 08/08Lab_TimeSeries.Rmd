...
#inner join - combine both data sets  
...
BR_rain_decomp_components <- as.data.frame(BR_rain_decomp$time.series)

BR_rain_decomp_components <- mutate(BR_rain_decomp_components,
        Observed = BR_complete$Rainfall,     
        Date = BR_complete$Date)

# Exercise 10: Visualize how the trend maps onto the data for both series

BR_rain_plot <- ggplot(BR_rain_decomp_components) +
  geom_line(aes(y = Observed, x = Date)) +
  geom_line(aes(y = trend, x = Date), color = "yellow")+
  ylab("Rainfall")
