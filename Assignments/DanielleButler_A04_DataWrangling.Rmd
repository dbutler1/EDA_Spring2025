...
>All four datasets should have the same number of columns but unique record counts (rows). Do your datasets follow this pattern? Yes, there are 20 columns in each dataset with unique row amounts.
...
> Answer: na.omit got rid of more rows - it dropped to 101 vs the drop_na was at 182. na_omit will remove rows if any column is NA, whereas drop_na will drop only for specifically the column in the function.
